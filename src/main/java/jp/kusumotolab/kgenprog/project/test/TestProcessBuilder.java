...
public class TestProcessBuilder {
	...
	public TestResults start(final GeneratedSourceCode generatedSourceCode) {
		...
		final ProcessBuilder builder = new ProcessBuilder(javaBin, "-cp", classpath, testExecutorMain, "-b",
				workingDir.toAbsolutePath().toString(), "-s", targetFQNs, "-t", testFQNs);
	...
	private Set<FullyQualifiedName> getTargetFQNs(final BuildResults buildResults) {
		final Set<FullyQualifiedName> sourceFQNs = getFQNs(buildResults, this.targetProject.getSourceFiles());
	...
	private Set<FullyQualifiedName> getFQNs(final BuildResults buildResults, final List<SourceFile> sources) {
		return sources.stream().map(source -> buildResults.getPathToFQNs(source.path)).flatMap(c -> c.stream())
				.collect(toSet());
	}
	...
	private String filterClasspathFromSystemClasspath() {
		...
		final List<String> result = Stream.of(classpaths)
				.filter(cp -> filter.stream().anyMatch(f -> cp.matches(".*" + f + jarFileTail))).collect(toList());
